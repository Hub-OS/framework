...
impl GameWindowLoop for Sdl2GameLoop {
    ...
    fn run(
        ...
        Box::new(async {
            ...
            game_loop.run(params).await
        })
    }
}

impl Sdl2GameLoop {
    async fn build(window_config: GameWindowConfig<()>) -> anyhow::Result<Sdl2GameLoop> {
        ...
        let window = Sdl2GameWindow::from_window_and_config(sdl_window, window_config).await?;
